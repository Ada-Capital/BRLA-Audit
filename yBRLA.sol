...
contract yBRLA is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, ERC20PausableUpgradeable, AccessControlUpgradeable, ERC20PermitUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address minter, address compliance, address distributor, address upgrader, address _brla)
        initializer public
    {
        __ERC20_init("Yield BRLA", "yBRLA");
        __ERC20Burnable_init();
        __ERC20Pausable_init();
        __ERC20Permit_init("Yield BRLA");
    ...
    function mint(uint256 brlaAmount) public onlyRole(MINTER_ROLE) {
        int128 currentPriceInv = _currentPriceInv();
        IERC20(brla).safeTransferFrom(_msgSender(), address(this), brlaAmount);
        uint256 yBrlaAmount = ABDKMath64x64.mulu(currentPriceInv, brlaAmount);
        _mint(_msgSender(), yBrlaAmount);
    }

    function unstake(address from, address to, uint256 yBrlaAmount) public {
        int128 currentPrice = _currentPrice();
        if (_msgSender() == from) {
            burn(yBrlaAmount);
        } else {
            burnFrom(from, yBrlaAmount);
        }
        uint256 brlaAmount = ABDKMath64x64.mulu(currentPrice, yBrlaAmount);
        IERC20(brla).safeTransfer(to, brlaAmount);
    }

    function unstakeWithPermit(address from, address to, uint256 yBrlaAmount,
        uint256 deadline,
        uint8 v,
        bytes32 r,
        bytes32 s) public {
        ...
        if (_msgSender() == from) {
            ...
            permit(from, _msgSender(), yBrlaAmount, deadline, v, r, s);
            burnFrom(from, yBrlaAmount);
        }
        uint256 brlaAmount = ABDKMath64x64.mulu(currentPrice, yBrlaAmount);
        IERC20(brla).safeTransfer(to, brlaAmount);
